...
Sub CopyFrom稼働疎通Toサブ間()
  ...
  copy_column_list = Array("C:H", "AS:AS", "AT:AY")
  ...
  For Each orig_key_cell In iteration
    ...
    Set orig_copy_range = Intersect(orig_key_cell.EntireRow, sheet_from.Range(copy_column_list(0)))
    For Each orig_copy_col In copy_column_list
        Set orig_copy_range = Union(orig_copy_range, Intersect(orig_key_cell.EntireRow, sheet_from.Range(orig_copy_col)))
    Next orig_copy_col
